<template>
    ...
    >
      <loading-spinner></loading-spinner>
        ...
        <div>
            <div class="mt-4 flex max-h-96 w-full justify-center gap-4 font-oswald">
                <dropdown class="ml-4 mr-auto self-start" :selected-tab="currentTab">
                    <li class="px-4 py-1 transition hover:bg-slate-600" v-for="(element, index) in dropdownElements"
                        :key="index" @click="handleDropdownSelect(index)">
                        {{ element }}
                    </li>
                </dropdown>
                <div class="my-2 mr-auto flex gap-20">
                    ...
                    <div class="mr-auto flex gap-20">
                        <div class="flex flex-col items-center justify-center text-center">
                            <img class="h-36 self-center" :src="
                                getImagePath(
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                <span class="font-bold tracking-wider" :class="
                                    colorLookup[RABenchmarks.overallRank]
                                ">{{ RABenchmarks.overallPoints }}</span>
                            </div>
                        </div>
                        <div class="flex flex-col items-center justify-center tracking-wide text-slate-200">
            ...
            <section class="relative p-4" id="benchmark-table">
                ...
                <div v-for="(bench, index) in RABenchmarks.benchmarks" :key="index"
                    ...
                    <div class="relative col-span-3 flex items-center justify-between">
                        <span class="absolute left-1/2 z-10 -translate-x-1/2 transform text-right"
                            v-if="bench.points">{{ bench.points }}</span>
                        <progress-bar class="h-5 w-full rounded-sm bg-slate-600" :value="bench.progress"
                            color="bg-sky-700"></progress-bar>
                    ...
                    <p class="relative col-start-12 ml-4 flex items-center justify-center">
                        <chevron-icon direction="down"
                            class="absolute h-full w-12 transition hover:translate-y-0.5 hover:text-slate-400"
                            :class="bench.detailsOpen ? 'rotate-180' : ''"
                            @click="toggleBenchDetails(bench)"></chevron-icon>
                    </p>
                    <div class="col-span-12 mt-2 border-t-2 border-t-slate-600 py-2 text-slate-200 transition"
                        v-if="bench.detailsOpen">
                        <div class="pb-4">
                            <div class="flex flex-col gap-0.5">
                                <div class="min-w-3/4 grid gap-0.5 text-center" :class="scoreReqGrid">
                                    <span v-for="(rank, index) in rankList" :key="index"
                                        class="bg-slate-700 px-8 py-2 text-center" :class="colorLookup[rank]">{{
                                            rank
                                        }}</span>
                                </div>
                                <div class="grid gap-0.5 text-center" :class="scoreReqGrid">
                                    <span v-for="(score, index) in bench.scores" :key="index"
                                        class="bg-slate-700 px-8 py-2">{{ score }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="flex gap-10 pl-4">
                            ...
                            <div class="ml-auto mr-10 flex items-center gap-10 text-white">
                                <button class="flex cursor-pointer items-center gap-1 transition hover:text-slate-300"
                                    @click="handlePlayScenario(bench.id)">
                                    <play-icon class="h-5 w-5 transition"></play-icon>Play
                                </button>
                                <button class="flex items-center" :class="
                                    bench.count == 0
                                        ? 'disabled pointer-events-none text-slate-500'
                                        : ''
                                " @click="replayLink(bench.id, bench.weapon)">
                                    <span v-if="replayLoading">Loading...</span>
                                    <span v-else class="cursor-pointer transition hover:text-slate-300">Watch
                                        Replay</span>
                                </button>

                                <router-link class="cursor-pointer transition hover:text-slate-300"
                                    :to="'/tasks/' + bench.id">View Leaderboard</router-link>
                            </div>
                        </div>
                    </div>
                </div>
